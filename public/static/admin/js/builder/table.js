...
jQuery(document).ready(function() {
    ...
    $.fn.editable.defaults.url = ieasynet.quick_edit_url;
    ...
    $.fn.editable.defaults.params = function (params) {
        ...
        params.validate = ieasynet.validate;

        params.validate_fields = ieasynet.validate_fields;
    ...
    var goto = function (url, _curr_params) {
        ...
        if ($.isEmptyObject(ieasynet.curr_params)) {

            params = jQuery.param(_curr_params);

        } else {

            $.extend(ieasynet.curr_params, _curr_params);

            params = jQuery.param(ieasynet.curr_params);
    ...
    var search_field = ieasynet.search_field;
    ...
    $('.table-builder .field-filter').click(function () {
        ...
        var self             = $(this),
            ...
            $_filter         = ieasynet._filter,

            $_filter_content = ieasynet._filter_content,

            $_field_display  = ieasynet._field_display,
        ...
        layer.open({
            ...
            success: function () {
                ...
                $.post(ieasynet.get_filter_list, $data).success(function(res) {
            ...
            yes: function () {
                ...
                goto(ieasynet.curr_url, _curr_params);
    ...
    $('.table-builder .switch input:checkbox').on('click', function () {
        ...
        $.post(ieasynet.quick_edit_url, $data).success(function(res) {
            Ieasynet.loading('hide');
            if(res.opt == 'isvip'){
                Ieasynet.notify('操作成功', 'success');
                location.reload();
                return false;
            }
            if (1 != res.code) {
    ...
    $('.pop').click(function () {
        ...
        if ($layer !== undefined) {
            ...
            $.extend($options, ieasynet.layer, $layer);

        } else {

            $.extend($options, ieasynet.layer);
