...
jQuery(document).ready(function() {
    ...
    $('.js-jcrop-interface').each(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.code == 1) {
                                ...
                                Ieasynet.notify('上传失败，请重新上传', 'warning');
                            }
                        }
                    }).fail(function() {
                        Ieasynet.loading('hide');
                        Ieasynet.notify('服务器错误~', 'danger');
                    });
                    $jcrop_file.val('');
                } else {
                    Ieasynet.notify('请选择一张图片', 'warning');
                }
            }
        });
        ...
        $jcrop_cut_btn.click(function () {
            ...
            if ($cut_value != '') {
                ...
                .fail(function() {
                    Ieasynet.loading('hide');
                    Ieasynet.notify('请求失败', 'danger');
                });
            } else {
                Ieasynet.notify('请选择要裁剪的大小', 'warning');
            }
        });
    ...
    $('.js-ueditor').each(function () {
        ...
        ueditors[ueditor_name] = UE.getEditor(ueditor_name, {
            ...
            serverUrl: ieasynet.ueditor_upload_url
        });
    });
    ...
    $('.js-wangeditor').each(function () {
        ...
        // 上传图片地址
        wangeditors[wangeditor_name].config.uploadImgUrl = ieasynet.wangeditor_upload_url;
        // 允许上传图片后缀
        ...
        wangeditors[wangeditor_name].config.emotions = {
            'default': {
                title: '默认',
                data: ieasynet.wangeditor_emotions
            }
        };
    ...
    $('.js-upload-file,.js-upload-files').each(function () {
        ...
        var uploader = WebUploader.create({
            ...
            duplicate: true,
            // swf文件路径
            swf: ieasynet.WebUploader_swf,
            // 文件接收服务端。
            server: ieasynet.file_upload_url,
            // 选择文件的按钮。可选。
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Ieasynet.notify('文件类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Ieasynet.notify('文件不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    $('.js-upload-image,.js-upload-images').each(function () {
        ...
        var uploader = WebUploader.create({
            ...
            swf: ieasynet.WebUploader_swf,
            // 图片接收服务端。
            server: ieasynet.image_upload_url,
            // 选择图片的按钮。可选。
            // 内部根据当前运行是创建，可能是input元素，也可能是flash.
            pick: {
                id: '#picker_' + $input_file_name,
        ...
        uploader.on('error', function (type) {
            switch (type) {
                case 'Q_TYPE_DENIED':
                    Ieasynet.notify('图片类型不正确，只允许上传后缀名为：'+$ext+'，请重新上传！', 'danger');
                    break;
                case 'F_EXCEED_SIZE':
                    Ieasynet.notify('图片不得超过'+ ($size/1024) +'kb，请重新上传！', 'danger');
                    break;
            }
        });
    ...
    // 表单项依赖触发
    if (ieasynet.triggers != '') {
        /* 依赖显示 */
        // 先隐藏依赖项
        var $field_hide   = ieasynet.field_hide.split(',') || [];
        var $field_values = ieasynet.field_values.split(',') || [];
        ...
        $.each(ieasynet.triggers, function (trigger, content) {
