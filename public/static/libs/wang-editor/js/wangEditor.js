    ...
    E.plugin(function () {
        ...
        editor.xhrUploadImg = function (opt) {
            ...
            if (editor.config.imgExt) {
                ...
                var result = false;
                for (var ext in imgExt) {
                    if (imgExt[ext] == fileExt) {
                        result = true;
                        break;
                    }
                }
                if (result === false) {
                    alert("只允许上传后缀名为："+editor.config.imgExt + '的图片');
                    return false;
                }
            }
            ...
            fileName = fileName || E.random() + '.' + fileExt;
            formData.append(name, convertBase64UrlToBlob(base64, fileType), fileName);
