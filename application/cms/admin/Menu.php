    ...
    public function add($nid = null, $pid = 0)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($menu = MenuModel::create($data)) {
                ...
                $this->success('新增成功', url('cartoon', ['id' => $nid]));
            } else {
                $this->error('新增失败');
            }
        }
        ...
        return ZBuilder::make('form')
            ->addFormItems([
                ...
                ['text', 'url', 'URL', "<code>必填</code>。如果是模块链接，请填写<code>模块/控制器/操作</code>，如：<code>admin/menu/add</code>。如果是普通链接，则直接填写url地址，如：<code>http://www.huotun.com</code>"],
                ['text', 'css', 'CSS类', '可选'],
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (MenuModel::update($data)) {
                ...
                $this->success('编辑成功', url('cartoon', ['id' => $data['nid']]));
            } else {
                $this->error('编辑失败');
            }
        }
