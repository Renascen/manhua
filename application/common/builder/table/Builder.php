...
class Builder extends ZBuilder
{
    ...
    public function addRightButton($type = '', $attribute = [], $pop = false)
    {
        ...
        $btn_attribute['style'] = 'font-size: 0.4em; padding: 0px 4px; margin:0px';
        // 合并自定义属性
        if ($attribute && is_array($attribute)) {
            $btn_attribute = array_merge($btn_attribute, $attribute);
        }
    ...
    /**
     * 设置Tab按钮列表
     * @param array $tab_list Tab列表  ['title' => '标题', 'href' => 'http://www.huotun.com']
                    ...
                    switch ($column['type']) {
                        ...
                        case 'pictureimg': // 单张图片
                            if (empty($row[$column['name']])) {
                                ...
                                data-pk="'.$row['_primary_key_value'].'" 
                                data-name="'.$_name.'">'.'
                                <img class="image" src="'.$tempimgurl.'">'.'<br>编辑</a>';
                            }
                            break;
                        case 'img_url': // 外链图片
                        ...
                        case 'picture': // 单张图片
                            ...
                            // }else{
                                // $row[$column['name']] = '<div class="js-gallery"><a href="'. $row[$column['name']].'" class="img-link" title="'.$row[$column['name']].'"><img class="image" src="'.$row[$column['name']].'"></a></div>';
                            // }
                            break;
                        case 'pictures': // 多张图片
                            if ($row[$column['name']] === '') {
