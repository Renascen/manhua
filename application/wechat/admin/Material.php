    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($post['type'] == 'news') { // 修改单篇图文
                ...
                $data['name'] = $post['name'];
                $data['content'] = json_encode($news_data);
                if (WeMaterialModel::update($data, array('id' => $post['id']))) {
...
EOF;
    ...
    public function delete($ids = '')
    {
        ...
        foreach ($ids as $id) {
            $we_reply = WeReplyModel::where(['type' => 'news', 'content' => $id])->field('id')->find();
            $where = '`type`="news" and (content like "%,'.$id.'%" or content like "%'.$id.',%" or content like "%,'.$id.',%")';
            $multi = WeReplyModel::whereOr($where)->value('id');
            $lastSql = DB::getLastSql();
            if ($we_reply) {
                $this->error('删除失败: 自动回复中( ID: ' . $we_reply['id'] . ' )包含此素材( ID: ' . $id . ' )', cookie('__forward__'));
            }elseif($multi){
                $this->error('删除失败: 自动回复中( ID: ' . $multi . ')包含此素材( ID: ' . $id . ')', cookie('__forward__'));
            }
            $info = WeMaterialModel::get($id);
            if($info['type'] == 'news'){
                $result = DB::table('ien_we_material')->where('id', $id)->delete();
                if($result){
                    $this->success('删除成功');
                }else{
                    $this->error('删除失败');
