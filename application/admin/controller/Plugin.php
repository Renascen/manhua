...
class Plugin extends Admin
{
    ...
    public function index($group = 'local', $type = '')
    {
        ...
        foreach ($list_group as $key => $value) {
            ...
            $tab_list[$key]['url']   = url('cartoon', ['group' => $key]);
        }
    ...
    public function manage($name = '')
    {
        ...
        // 加载自定义后台页面
        if (plugin_action_exists($name, 'Admin', 'cartoon')) {
            return plugin_action($name, 'Admin', 'cartoon');
        }
    ...
    public function config($name = '')
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (false !== PluginModel::where('name', $name)->update(['config' => $data])) {
                return $this->success('更新成功', 'cartoon');
            } else {
